    ...
    async for session in get_db():
        ...
        if not marathon:
            ...
            part_count = await session.scalar(
                ...
                .where(MarathonParticipant.marathon_id == marathon.id, MarathonParticipant.is_active)
            )
...
async def process_points_emoji(message: types.Message, state: FSMContext) -> None:
    ...
    async for session in get_db():
        ...
        await session.execute(
            update(Marathon)
            .where(Marathon.curator_id == user_id, Marathon.is_active)
...
async def marathon_invite_generate(event, state: FSMContext) -> None:
    ...
    async for session in get_db():
        ...
        if days == 0:
            ...
            marathon.invite_token_expires_at = datetime.now() + timedelta(days=days)
