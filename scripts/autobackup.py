...
async def main():
    ...
    try:
        ...
        # 5. Send Notification with DB File
        ...
        await bot.send_document(
            chat_id=admin_id,
            document=FSInputFile(str(db_file)),
            caption=f"🗗 DB Backup: {timestamp}"
        )
        await bot.session.close()
        ...
        try:
            bot = Bot(token=settings.BOT_TOKEN)
            await bot.send_message(chat_id=settings.ADMIN_IDS[0], text=f"❌ **Backup Critical Failure**: {e}")
            await bot.session.close()
        except Exception:
            pass
