...
async def test_get_water_advice_no_last_food(mocker):
    ...
    session_mock.execute.side_effect = [mock_settings_result, mock_water_result, mock_food_result, MagicMock()]
...
async def test_get_contextual_advice_presets(mocker):
    ...
    # 7. Fridge (scalars.all)
    mock_fridge_result = MagicMock()
    mock_fridge_result.scalars().all.return_value = []

    # 8. Tokens sum
    mock_tokens_result = MagicMock()
    mock_tokens_result.scalar.return_value = 5000
